...
if st.button(" 🔍 CALCULAR"):
    ...
    previsao = model.predict(input_values_reshaped)
    proba = model.predict_proba(input_values_reshaped)
...
with st.container(border=True):
        if calculado:
            st.markdown(f'''#### Para os dados inseridos o resultado é e a probabilidade desse resultado ser verdadeiro é de {round(porcentagem, 2)}% ''')
            
        st.markdown('---------')
with st.container(border=True):
        st.markdown(
            '''⚠️ ATENÇÃO: Este modelo não tem a pretensão de servir como diagnótico, ele foi desenvolvido
            com base dados públicos que englobam fatores de predisposição a problemas cardíacos. 
            '''
            )
